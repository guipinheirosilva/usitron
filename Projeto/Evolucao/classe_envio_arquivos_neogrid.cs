...
namespace Evolucao
{
    public class classe_envio_arquivos_neogrid
    {
        ...
        string dtTermino = DateTime.Now.ToShortDateString();

        public classe_envio_arquivos_neogrid(string data_inicio, string data_final)
        {
            this.dtInicio = data_inicio;
            this.dtTermino = data_final;
        ...
        public void gerar_csv_vendas()
        {
            ...
            if (criar_arquivo(caminho))
            {
                ...
                { //13324184000197
                    ...
                    texto.WriteLine("PERIODO" + ";" + Convert.ToDateTime(dtInicio).ToShortDateString() + ";" + "\tA" + ";" + Convert.ToDateTime(dtTermino).ToShortDateString());
                    texto.WriteLine("CODIGO" + ";" + "EAN" + ";" + "DESCRICAO" + ";" + "QTDE VENDIDO" + ";" +
                           "VALOR VENDIDO" + ";" + "QTDE CANCELADO" + ";" + "VALOR CANCELADO" + ";" + "QTDE BONIFICADO" + ";" + "VALOR BONIFICADO");
                    FbCommand select = new FbCommand();
                    select.Connection = fbConnection1;
                    select.CommandText =
...
"	or infe1.cfop_item = '5405')	" +
 " and nf1.data_emissao_nf >= '" + Convert.ToDateTime(dtInicio).ToShortDateString().Replace("/", ".") + "' " +
 "and nf1.data_emissao_nf <= '" + Convert.ToDateTime(dtTermino).ToShortDateString().Replace("/", ".") + "' " +
...
"	or infe2.cfop_item = '5405')	" +
 " and nf2.data_emissao_nf >= '" + Convert.ToDateTime(dtInicio).ToShortDateString().Replace("/", ".") + "' " +
 "and nf2.data_emissao_nf <= '" + Convert.ToDateTime(dtTermino).ToShortDateString().Replace("/", ".") + "' " +
...
"	or infe3.cfop_item = '5405')))	" +
 " and nf3.data_emissao_nf >= '" + Convert.ToDateTime(dtInicio).ToShortDateString().Replace("/", ".") + "' " +
 "and nf3.data_emissao_nf <= '" + Convert.ToDateTime(dtTermino).ToShortDateString().Replace("/", ".") + "' " +
...
"	or infe4.cfop_item = '5405')))	" +
 " and nf4.data_emissao_nf >= '" + Convert.ToDateTime(dtInicio).ToShortDateString().Replace("/", ".") + "' " +
 "and nf4.data_emissao_nf <= '" + Convert.ToDateTime(dtTermino).ToShortDateString().Replace("/", ".") + "' " +
...
"	or infe5.cfop_item = '6910')	" +
 " and nf5.data_emissao_nf >= '" + Convert.ToDateTime(dtInicio).ToShortDateString().Replace("/", ".") + "' " +
 "and nf5.data_emissao_nf <= '" + Convert.ToDateTime(dtTermino).ToShortDateString().Replace("/", ".") + "' " +
...
"	or infe6.cfop_item = '6910')	" +
 " and nf6.data_emissao_nf >= '" + Convert.ToDateTime(dtInicio).ToShortDateString().Replace("/", ".") + "' " +
 "and nf6.data_emissao_nf <= '" + Convert.ToDateTime(dtTermino).ToShortDateString().Replace("/", ".") + "' " +
...
" and nf.n_protocolo_autorizacao_nf is not null " +
 " and nf.data_emissao_nf >= '" + Convert.ToDateTime(dtInicio).ToShortDateString().Replace("/", ".") + "' " +
 "and nf.data_emissao_nf <= '" + Convert.ToDateTime(dtTermino).ToShortDateString().Replace("/", ".") +
            ...
            if (criar_arquivo(caminho))
            {
                ...
                { //13324184000197
                   ...
                   select.CommandText =
                    ...
                    foreach (DataRow dr in dtSelect.Rows)
                    {
                        string data_vendedor = retirar_caracteres(data_documento.ToString("yyyyMMdd"));
                        string status = "A";
                        string teste = dr["data_demissao_vendedor"].ToString();
                        if (dr["data_demissao_vendedor"].ToString() != null && dr["data_demissao_vendedor"].ToString() != "")
                        {
                            data_vendedor = Convert.ToDateTime(dr["data_demissao_vendedor"]).ToShortDateString().Replace("/", "");
                            status = "I";
                        }

                        texto.WriteLine("02|" + dr["NOME_VENDEDOR"].ToString() + "|" + dr["COD_VENDEDOR"].ToString() + "|NOME_SUPERVISOR|1|NOME_GERENTE|1|" + status + "|" + data_vendedor + "|");
                    }
                }
            }
        }

        public void gerar_vendas()
        {
            DateTime data_documento = DateTime.Now;
            ...
            if (criar_arquivo(caminho))
            {
                ...
                using (StreamWriter texto = t.CreateText())
                {
                    texto.WriteLine("01|VENDAS|051|1|" + retirar_caracteres(data_documento.ToString("yyyyMMddHHmm")) + "|" +
                       retirar_caracteres(Convert.ToDateTime(dtInicio).ToString("yyyyMMdd")) + "|" +
                       retirar_caracteres(Convert.ToDateTime(dtTermino).ToString("yyyyMMdd")) + "|" +
                    ...
                    {//yyyy-MM-ddTHH:mm:ss
                        ...
                        foreach (DataRow dr_itens in dtSelect_itens.Rows)
                        {
                            ...
                            {
                                bonificacao = "S";
                                dr_itens["PRECO_UNIT_ITEM"] = "0,00";
                                dr_itens["PRECO_TOTAL_ITEM"] = "0,00";
                            }
       ...
       out string senha_email_nfe_empresa, out string logotipo_empresa, out string proxy_empresa, out string usuario_proxy_empresa, out string senha_proxy_empresa, out string crt_empresa, out string serie_empresa)
        {
            ...
            try
            {
                ...
                comando.CommandText = "SELECT * FROM EMPRESA where LICENCA_DLL_NFE_EMPRESA is not null and LICENCA_DLL_NFE_EMPRESA <> ''";
            ...
            catch (Exception ex)
            {
                ...
                fe.erro = "Erro ao buscar dados da empresa";
                fe.ShowDialog();*/
            }
        }
