...
namespace Evolucao
{
    class classeTributacao
    {
        ...
        public string classificacaoIBSCBS;
        ...
        internal void cst900()
        {
            ...
            if (pMVAST > 0)
                ...
                vBCST = 0;
            double valor_temp_icms_st = vBCST * pICMSST / 100;
            if (valor_temp_icms_st > 0)
                vICMSST = valor_temp_icms_st - vICMS;
            else
                vICMSST = 0;
        }

        public void cst51() //DIFERIMENTO
        {
            //DIFERIMENTO 100%. Se fosse menos teria outras variáveis
            vBC = 0;// vBC - (vBC * pRedBC / 100);
            vICMS = 0; //vBC * pICMS / 100;
            if (vICMS == 0)
                vBC = 0;
        ...
        public void inserir_informacoes_item(string item, string cfop)
        {
            try
            {
                ...
                string _orig = "null", _CST = "null", _modBC = "null",
                    ...
                    _BCISS = "null", _pISS = "null", _vISS = "null", _tipoISS = "null",
                      _pIBS = "null", _vIBS = "null", _pCBS = "null", _vCBS = "null", _cstIBSCBS = "null", _classificacaoIBSCBS = "null", _BCIBS = "null";
                ...
                {
                    if (vBC > 0)
                        _vBC = "'" + vBC.ToString("n2").Replace(".","").Replace(",",".") + "'"; // campo double                    
                }
                catch { }
                try
                {
                ...
                {
                    if (vICMS > 0)
                        _vICMS = "'" + vICMS.ToString("n2").Replace(".", "").Replace(",", ".") + "'";
                }
                catch { }
                try
                {
                ...
                string zerar_valores = "";
                if(cfop == "1604")
                {
                    zerar_valores = " , PRECO_UNIT_ITEM = 0, PRECO_TOTAL_ITEM = 0 ";
                }
                ...
                {
                    if (cstIBSCBS != "")
                        _cstIBSCBS = "'" + cstIBSCBS + "'";
                }
                catch { }
                try
                {
                    if (classificacaoIBSCBS != "")
                        _classificacaoIBSCBS = "'" + classificacaoIBSCBS + "'";
                }
                catch { }
                try
                {
                    if (BCIBS > 0)
                        _BCIBS = "'" + BCIBS.ToString().Replace(".", "").Replace(",", ".") + "'";
                }
                catch { }
                try
                {
                    if (pIBS > 0)
                        _pIBS = "'" + pIBS.ToString().Replace(".", "").Replace(",", ".") + "'";
                }
                catch { }
                try
                {
                    if (vIBS > 0)
                    {
                        _vIBS = "'" + vIBS.ToString().Replace(".", "").Replace(",", ".") + "'";
                    }
                    else { _vIBS = "'0'"; }
                }
                catch { }
                ...
                {
                    if (pCBS > 0)
                        _pCBS = "'" + pCBS.ToString().Replace(".", "").Replace(",", ".") + "'";
                }
                catch { }
                try
                {
                    if (vCBS > 0)
                    {
                        _vCBS = "'" + vCBS.ToString().Replace(".", "").Replace(",", ".") + "'";
                    }
                    else { _vCBS = "'0'"; }
                }
                catch { }
                ...
                comando_select.CommandText =
                    ...
                    "TIPO_SERVICO_ITEM = " + _tipoISS + ", " + 
                       //IBS
                    "CST_IBSCBS_ITEM =" + _cstIBSCBS + ", " +
                    "CCLASSTRIB_ITEM =" + _classificacaoIBSCBS + ", " +
                    "BC_IBSCBS_ITEM =" + _BCIBS + ", " +
                    "ALIQ_IBS_ITEM =" + _pIBS + ", " +
                    "ALIQ_CBS_ITEM =" + _pCBS + ", " +
                    "VALOR_IBS_ITEM =" + _vIBS + ", " +
                    "VALOR_CBS_ITEM =" + _vCBS + " " +zerar_valores + 
        ...
        internal void calcular_IBSCBS()
        {
            vIBS = Math.Round(BCIBS * pIBS / 100, 2, MidpointRounding.AwayFromZero);
            vCBS = Math.Round(BCIBS * pCBS / 100, 2, MidpointRounding.AwayFromZero);
        }
    }
}
